...
class SuccessFilesPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final addResource = () {
      ...
      showDialog(barrierDismissible: false, context: context, builder: (BuildContext context) {return AddRessourceDialog();});
      
    };
    
    
    if (! state.isRootPath())  cards.add(Card(child: SizedBox(child: IconButton(icon: Icon(Icons.arrow_back), onPressed: backTap)),)) ;

    //refresh arrow
    cards.add(Card(child: SizedBox(child: IconButton(icon: Icon(Icons.refresh), onPressed: refreshTap)),));
    ...
    //add someting
    if (! state.isRootPath()) cards.add(Card(child: SizedBox(child: IconButton(icon: Icon(Icons.add), onPressed: addResource)),));


    return LayoutBuilder(
      builder: (BuildContext context , BoxConstraints constraints) {
        final isMobile = constraints.maxWidth <= 650;
        final isSuperthin = constraints.maxWidth <= 400;
        return Container(
          ...
          child: GridView.count(
            ...
            crossAxisCount: isSuperthin ? 1 : isMobile ? 2 : 3 ,
            children: cards,
            ),
        );
      }
